...
Each `trusted_authorities` entry is a `{ "type": ..., "values": [...] }` object (some types add a type-specific member, e.g. `trqp_v2`'s `endpoint`). How an Agent can resolve it for acquisition depends on its `type`:
...
- **`trqp_v2`** — the entry has a REQUIRED `endpoint` (the HTTPS base URL of a [ToIP TRQP v2.0](https://trustoverip.github.io/tswg-trust-registry-protocol/approved/) registry, e.g. `https://trust.example.org/trqp/v2/`) and an OPTIONAL `values` array of the `authority_id`s it governs. To confirm or discover an authorized issuer, a client `POST`s `<endpoint>/authorization` (or `/recognition`) with `authority_id`, the issuer as `entity_id`, and the `action` and `resource` in the JSON body, then drives OpenID4VCI against an authorized issuer. If `values` is omitted, the `authority_id` is derived from `resource` — allowed only when `resource` is a registry-unique schema identifier. Unlike the other types, TRQP returns an authoritative yes/no rather than only listing issuers; whether an authorized issuer exposes OpenID4VCI is an ecosystem convention, so acquisition is best-effort.
